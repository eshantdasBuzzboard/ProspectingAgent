import pandas as pd
from core.chains.business_intelligence_chains import get_default_filter_prospects
...
async def get_propsects_default(search_query, state, cateogry, products) -> dict:
    ...
    results = await get_default_filter_prospects(
        business_info=business_info, products_info=products, search_query=search_query
    )
